...
grayImage = im2gray(I);
[y,x,m] = harris(grayImage, 2000,'thresh',0.1,'tile',[2 2],'sigma',5,'hsize',45,'eig','fft');
points = [x y];
...
for n = 2:numImages
    ...
    [y,x,m] = harris(grayImage, 2000,'thresh',0.1,'tile',[2 2],'sigma',5,'hsize',45,'eig','fft');
    points = [x y];
    % features = convolve2(grayImage, M, 'same', 0.2);
    [features, points] = extractFeatures(grayImage, points);
  
    % Find correspondences between I(n) and I(n-1).
    [indexPairs, matchm] = matchFeatures(features, featuresPrevious, 'Unique', true);
